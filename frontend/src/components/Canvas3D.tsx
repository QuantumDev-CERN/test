...
// --- Constants ---
const NODE_COUNT = 120; // Stays 120

// --- Keep the large bounding box ---
const BOUNDING_BOX_X = 22; // Stays wide
const BOUNDING_BOX_Y = 20; // Stays tall
const BOUNDING_BOX_Z = 8;  // Stays the same.

// --- CHANGE 1: Decrease connection threshold to reduce line density ---
const CONNECTION_DISTANCE_THRESHOLD = 4.5; // Was 6
...
const useBlockchainData = () => {
  return useMemo(() => {
    ...
    const basePositions = new Float32Array(NODE_COUNT * 3); // Updated size

    // --- Use the new X, Y, Z constants for generation ---
    for (let i = 0; i < NODE_COUNT; i++) { // Uses new count
    ...
    for (let i = 0; i < NODE_COUNT; i++) { // Uses new count
      for (let j = i + 1; j < NODE_COUNT; j++) { // Uses new count
        const posA = positions[i];
        const posB = positions[j];
        const dist = posA.distanceTo(posB);

        if (dist < CONNECTION_DISTANCE_THRESHOLD) { // Uses new threshold
...
function BlockchainNodes({ nodePositions }: { nodePositions: THREE.Vector3[] }) {
  ...
  return (
    <instancedMesh
      ref={meshRef}
      args={[undefined, undefined, NODE_COUNT]} // Uses new count
      frustumCulled={false}
    >
...
function Scene({ mouseTracking = false }: { mouseTracking?: boolean }) {
  ...
  return (
    <>
      <PerspectiveCamera
        makeDefault
        // --- Keep camera close and FOV wide ---
        position={[0, 0, 12]} // Stays 12
        fov={75} // Stays 75
        near={0.1}
        far={200}
      />
